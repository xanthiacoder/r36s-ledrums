...
local sceneNumber = 403

-- music data for this scene

-- filenames of samples to load
local sample = {
	[1] = "Gabber-RideCymbal.wav",
	[2] = "Gabber-Snare.wav",
	[3] = "Gabber-Drum1.wav",
	[4] = "Gabber-Drum2.wav",
	[5] = "Gabber-ClosedHiHat.wav",
	[6] = "Gabber-Clap.wav",
	[7] = "Gabber-OpenHiHat.wav",
	[8] = "Gabber-CrashCymbal.wav",
	}
...
function love.keyreleased( key, scancode )
   ...
   elseif scancode == "l" then
      ...
      bbtnState[1] = "up"
    	-- move to previous scene 402
      if love.keyboard.isScancodeDown("escape") then  -- SELECT + L1 release detected
      	saveData() -- force scene autosave
      	scene[402].input() -- change input key-map to 401's
      	scene[402].start() -- run 401 start process
      	scene.current = 402 -- change to 401 scene
		scene.previous = sceneNumber -- put current scene into scene history
      end
   ...
   elseif scancode == "r" then
      -- Right R1 released
    	-- move to next scene 404
      if love.keyboard.isScancodeDown("escape") then  -- SELECT + R1 release detected
      	saveData() -- force scene autosave
      	scene[404].input() -- change input key-map to 404's
      	scene[404].start() -- run 404 start process
      	scene.current = 404 -- change to 404 scene
		scene.previous = sceneNumber -- put current scene into scene history
      end
