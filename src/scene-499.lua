...
function K.start()
	...
	sfx[1] = love.audio.newSource("samples/wav/"..sample[1], "static")
	sfx[2] = love.audio.newSource("samples/wav/"..sample[2], "static")
	sfx[3] = love.audio.newSource("samples/wav/"..sample[3], "static")
	sfx[4] = love.audio.newSource("samples/wav/"..sample[4], "static")
	sfx[5] = love.audio.newSource("samples/wav/"..sample[5], "static")
	sfx[6] = love.audio.newSource("samples/wav/"..sample[6], "static")
	sfx[7] = love.audio.newSource("samples/wav/"..sample[7], "static")
	sfx[8] = love.audio.newSource("samples/wav/"..sample[8], "static")
...
function love.keyreleased( key, scancode )
   ...
   elseif scancode == "l" then
      -- Back L1 released
    	-- move to previous scene 498
      if love.keyboard.isScancodeDown("escape") then  -- SELECT + L1 release detected
      	saveData() -- force scene autosave
      	scene[498].input() -- change input key-map to 498's
      	scene[498].start() -- run 498 start process
      	scene.current = 498 -- change to 498 scene
		scene.previous = sceneNumber -- put current scene into scene history
      end
   ...
   elseif scancode == "r" then
      -- Right R1 released
    	-- move to next scene 401
      if love.keyboard.isScancodeDown("escape") then  -- SELECT + R1 release detected
      	saveData() -- force scene autosave
      	scene[401].input() -- change input key-map to 401's
      	scene[401].start() -- run 401 start process
      	scene.current = 401 -- change to 401 scene
		scene.previous = sceneNumber -- put current scene into scene history
      end
