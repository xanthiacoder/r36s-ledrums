-- scene-408
-- scene name : LEDrums (Industrial Madness)
-- scene functions are :
...
local sceneNumber = 408

-- music data for this scene

-- filenames of samples to load
local sample = {
	[1] = "Industrial-Hit1.wav",
	[2] = "Industrial-Snare.wav",
	[3] = "Industrial-Drum1.wav",
	[4] = "Industrial-Drum2.wav",
	[5] = "Industrial-FX1.wav",
	[6] = "Industrial-Drum3.wav",
	[7] = "Industrial-FX2.wav",
	[8] = "Industrial-Hit2.wav",
	}
...
function love.keyreleased( key, scancode )
   ...
   elseif scancode == "l" then
      -- Back L1 released
    	-- move to previous scene 407
      if love.keyboard.isScancodeDown("escape") then  -- SELECT + L1 release detected
      	saveData() -- force scene autosave
      	scene[407].input() -- change input key-map to 407's
      	scene[407].start() -- run 407 start process
      	scene.current = 407 -- change to 407 scene
		scene.previous = sceneNumber -- put current scene into scene history
      end
